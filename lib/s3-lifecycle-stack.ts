...
export class S3LifecycleStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const objectPutterFunction = new NodejsFunction(this, "ObjectPutterFunction", {
      entry: join(__dirname, "put-object.function.ts"),
      handler: "handler"
    });
    ...
    const eventNotificationProcessor = new NodejsFunction(this, "EventNotificationProcessorFunction", {
      entry: join(__dirname, "process-event-notification.function.ts"),
      handler: "handler"
    });
    ...
    const getMetricWidgetImageFunction = new NodejsFunction(this, "GetMetricWidgetImageFunction", {
      entry: join(__dirname, "get-metric-widget-image.function.ts"),
      handler: "handler",
      environment: {
    ...
    getMetricWidgetImageFunction.role!!.addToPrincipalPolicy(
      new PolicyStatement({
        effect: Effect.ALLOW,
